...
class receiver:
    ...
    def isCorrupted(self, packet):
        ...
        if(ccs + packet.seqNum + packet.ackNum == packet.checksum):
            return False
        else:
            return True
    ...
    def input(self, packet):
        ...
        if(self.isCorrupted(packet) or self.isDuplicate(packet)):
            seqNext = (self.seqnum+1)%2
            ackPacket = Packet(0,seqNext,seqNext,"")
            self.networkSimulator.udtSend(B,ackPacket)
        else:
            self.networkSimulator.deliverData(B,packet.payload)
            ackPacket = Packet(0,self.seqnum,self.seqnum,"")
            self.networkSimulator.udtSend(B,ackPacket)
